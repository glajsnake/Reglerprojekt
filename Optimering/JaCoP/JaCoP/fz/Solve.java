...
// 		costVariable = cost;
	...
	for (int i=0; i<dictionary.defaultSearchVariables.size(); i++)
	    if (dictionary.defaultSearchVariables.get(i) instanceof JaCoP.core.BooleanVar)
		bool_varSize++;
	    else
		int_varSize++;
	for (int i=0; i<dictionary.defaultSearchArrays.size(); i++)
	    if (dictionary.defaultSearchArrays.get(i)[0]  instanceof JaCoP.core.BooleanVar)
	...
	for (int i=0; i<dictionary.defaultSearchArrays.size(); i++)
		...
		Var v = dictionary.defaultSearchArrays.get(i)[j];
 		if (v  instanceof JaCoP.core.BooleanVar) 
...
// 	java.util.Arrays.sort(int_search_variables, new NumberConstraintsComparator<Var>());
...
//       	System.out.println(list_seq_searches);
    ...
    /*
    class NumberConstraintsComparator<T extends Var> implements Comparator<T> {

	NumberConstraintsComparator() { }

	public int compare(T o1, T o2) {
	    int v1 = o1.sizeConstraintsOriginal();
	    int v2 = o2.sizeConstraintsOriginal();
	    return v2 - v1;
	}
    }
    */
