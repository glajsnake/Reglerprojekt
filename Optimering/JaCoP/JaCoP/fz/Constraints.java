...
// 		    pose(new XmulYeqZ(t1, v2, t2));
		...
		if (p.startsWith("bool_", 6)) {
			...
			//	IntVar t1 = new Variable(store, 0, a1.length);
...
// 		    pose(new XplusYeqC(v1, v2, 1));
		...
		if (p.startsWith("eq", 4)) {
		    ...
		    PrimitiveConstraint c = new JaCoP.set.constraints.AeqB(v1, v2);
		    ...
		    pose(new Not(new JaCoP.set.constraints.AeqB(v1, v2)));
		}
		    ...
		    if (v1Type.getId() == JJTSETLITERAL ) {
		    	...
		    	IntVar v1 = getVariable(p1);
		    	c = new JaCoP.constraints.In(v1, d);
		    }
		    else {
			SetVar v2 = getSetVariable(node, 1);
...
// 				       ", "+ cost +")");   
		    ...
		    DecomposedConstraint c = new JaCoP.constraints.Lex(new IntVar[][] {x, y}, true);
		    ...
		    DecomposedConstraint c = new JaCoP.constraints.Lex(new IntVar[][] {x, y});
		    ...
		    int[] w = getIntArray((SimpleNode)node.jjtGetChild(2));
   		    ...
   		    pose( new JaCoP.constraints.binpacking.Binpacking(bin, capacity, w) );
...
//     }
	...
	if (p.startsWith("_reif", 10)) { // reified
	    ...
	    case le :
		if (p1.length == 2 && p1[0] == 1 && p1[1] == -1) 
		    pose(new Reified(new JaCoP.constraints.XplusClteqZ(p2[0], -p3, p2[1]), p4));
		else if (p1.length == 2 && p1[0] == -1 && p1[1] == 1) 
		    pose(new Reified(new JaCoP.constraints.XplusClteqZ(p2[1], -p3, p2[0]), p4));
		else if (p1.length == 1 && p1[0] == 1) 
		    pose(new Reified(new JaCoP.constraints.XlteqC(p2[0], p3), p4));
		else if (p1.length == 1 && p1[0] == -1) 
		    pose(new Reified(new JaCoP.constraints.XgteqC(p2[0], -p3), p4));
	    ...
	    case eq :
 		...
 		//     DecomposedConstraint c = new JaCoP.constraints.LinearEquationDomainConsistency(p2, p1, p3);
 		//     store.imposeDecomposition(c);
 		// }
 		// else
		if (p1.length == 2 && p1[0] == 1 && p1[1] == -1) {
	...
	//     rel[i][1] = p2[i];
	// }
	// pose(new JaCoP.constraints.ExtensionalSupportSTR(new IntVar[] {p1, p3}, rel));
